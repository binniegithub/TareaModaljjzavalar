...
function InsertarRegistro(){
    if(document.getElementById("IDTextBox").value!==null && document.getElementById("IDTextBox").value!==undefined && document.getElementById("IDTextBox").value.toString().length==14){
        ...
        d.innerHTML = document.getElementById("FirstNameTextBox").value;
        e.innerHTML = document.getElementById("LastNameTextBox").value;

        var genero = GetCheckedRadioValue("Sexo");
        f.innerHTML = genero;

        if(genero=="Masculino"){
            b.className = "gendermale";

        }
        else{
            b.className = "genderfemale";
        }

        g.innerHTML = GetCheckedBoxesValue("Pasatiempos");
...
function ShowModalForEdit(sender){
    ...
    var PasatiemposString = table.rows[selectedRowIndex].cells[5].innerHTML.toString();
...
function ActualizarRegistro(){
    ...
    if(genero=="Masculino"){
        table.rows[selectedRowIndex].cells[0].className = "gendermale";

    }
    else{
        table.rows[selectedRowIndex].cells[0].className = "genderfemale";
    }
    table.rows[selectedRowIndex].cells[4].innerHTML = GetCheckedRadioValue("Sexo");
    table.rows[selectedRowIndex].cells[5].innerHTML = GetCheckedBoxesValue("Pasatiempos");
    modal.style.display = "none";
}
